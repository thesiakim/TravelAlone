...
<!-- SQL 테이블 불러오기 -->
   <table>
   		<tr><th hidden></th></tr>
   		<tr><td width="200px;">숙소명</th>			<td>${house.h_name}</td></tr>
		<tr><td>숙소주소</td>						 	<td>${house.h_address}</td></tr>
		<tr><td>객실 가격</td>							<td>${house.h_room}</td></tr>
		<tr><td>체크인 체크아웃시간</td>					<td>${house.h_checkinout}</td></tr>
		<tr><td>문의전화</td>							<td>${house.h_call}</td></tr>
		<tr><td>조식여부</td>							<td>${house.h_eat}</td></tr>
		<tr><td>주차장여부</td>							<td>${house.h_parking}</td></tr>
		<tr><td>비고</td>								<td>${house.h_content}</td></tr>
		<tr><th hidden>								${house.house_id} </th>  </tr>
   </table>
   <br><br>
   
   <c:if test="${user_role == 'rol200' }">
      <a href="houUpdateForm?house_id=${house.house_id}"><input type="submit" value="수정"></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <a href="deleteHouse?house_id=${house.house_id}"><input type="submit" value="삭제"></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   </c:if>
  ...
  <tr>
    <c:forEach items="${mhHouImgList}" var="houImg">
      <td style="text-align: center;">
        ...
        <a href="javascript:void(0);" onclick="openPopup('${url}', '${houImg.img_id}')">
   ...
   <table style="margin:auto;" id="review">
               ...
               <td style=" padding-left:30px;" ><a href="houRevUpdateForm?house_id=${houRev.house_id}&review_id=${houRev.review_id}"><input type="submit" value="수정"></a></td>
                   <td style=" padding-left:30px;">
                    <a href="deleteHouRev?review_id=${houRev.review_id}" onclick="return confirm('정말로 삭제하시겠습니까?')"><input type="submit" value="삭제"></a>
                    </td>
                  </c:if>
