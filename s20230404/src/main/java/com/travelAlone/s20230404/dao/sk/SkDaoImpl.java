...
//			log.info("SkDaoImpl Exception"+ e.getMessage());
	...
	// 리뷰
	@Override
	public List<Res_Rev> selectResRevList(Res_Rev res_Rev) {
		List<Res_Rev> resRevList = new ArrayList<Res_Rev>();
		log.info("res_Rev.getRestaurant_id() ->" + res_Rev.getRestaurant_id() );
		try {
//			log.info("SkDaoImpl selectResRevList resRevList start");
			resRevList = session.selectList("resRevList",res_Rev);
...
//			log.info("SkDaoImpl delete Exception->"+ e.getMessage());
	...
	public int totalResRev(int rid) {
		int totalResRev = 0;
		log.info("SkDaoImpl Start total");
		log.info("SkDaoImpl rid->"+rid);
		try {
			totalResRev = session.selectOne("skResRevTotal",rid);
			log.info("SkDaoImpl totalResRev totalResRev->" + totalResRev);
		} catch (Exception e) {
			log.info("SkDaoImpl totalResRev Exception " +e.getMessage());
		}							
		return totalResRev;
	}
}
